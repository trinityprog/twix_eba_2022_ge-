<x-app-layout>
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        ...
        <form method="POST" action="{{ route('admin.rules.update', $lang) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label class="w-64 flex flex-col items-center px-4 py-6 bg-white rounded-md shadow-md tracking-wide uppercase border border-blue cursor-pointer hover:bg-purple-600 hover:text-white text-purple-600 ease-linear transition-all duration-150">
                    ...
                    <input type="file" class="hidden" id="rules_input" name="rules_{{ $lang }}" accept="application/pdf">
                </label>
                {!! $errors->first('rules_'.$lang, '<p class="mt-2 ml-2 text-xs text-red-600 dark:text-red-400">:message</p>') !!}
            </div>
